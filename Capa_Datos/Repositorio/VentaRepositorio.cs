...
namespace Capa_Datos.Repositorio
{
    public class VentaRepositorio : IVenta
    {
        ...
        public string GrabarVenta(int idCliente, decimal total, List<Detalle_Venta> detalle)
        {
                    ...
                    try
                    {
                        ...
                        foreach (var venta in detalle)
                        {
                            using (var comando = new SqlCommand("sp_AgregarVentaDetalle", conexion, transaccion))
                            {
                                ...
                                comando.Parameters.AddWithValue("@id_producto", venta.IdProducto);
                                comando.Parameters.AddWithValue("@id_servicio", venta.IdServicio);
        ...
        public List<Detalle_Venta> obtenerDetalleVentaPorIdVenta(int idVenta)
        {
            var listado = new List<Detalle_Venta>();
            ...
            using (var conexion = new SqlConnection(cadenaConexion))
            {
                ...
                using (var cmd = new SqlCommand("SP_ObtenerDetallesVentaPorIdVenta", conexion))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public VentaCabecera obtenerVentaCabecera(int idVenta)
        {
            VentaCabecera ventaCabecera = null;

            using (var conexion = new SqlConnection(cadenaConexion))
            {
                conexion.Open();
                using (var cmd = new SqlCommand("sp_obtenerVentaCabecera", conexion))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@id", idVenta);
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        if (reader != null && reader.HasRows)
                        {
                            ...
                            {
                                idVenta = Convert.ToInt32(reader["idCliente"]),
                                idCliente = Convert.ToInt32(reader["idVenta"]),
